...
////////////////
////////////////
//anstatt eines arduino uno wird jetzt ein espressif esp8266 dev board clone verwendet
//benötigt den arduino core for esp8266 wifi chip um mit der arduino ide zu funktionieren
//beim shield wurde der poti für den kontrast nach hinten verlegt und die icsp pins abgebrochen
//der button select wird nicht verwendet
/*Changelog
...
const byte bmpSDA = D2;
const byte bmpSCL = D3;
//tempmessung
...
byte hysterese = 2;  //für den thermostaten
...
//
//////////////////////////////////////////////////////////////////////////////////////Timer
int timerIntervall = 845; //korrigierte Sekunde
...
byte tempEinmaischen = 59;  //StartTemperaturen
byte tempStep1 = 57;   
byte tempStep2 = 64; 
byte tempStep3 = 72; 
byte tempStep4 = 78; 
        ...
        if (!hMenueON && nMenueON && hMenue == 1) {
          ...
          if (!meldung && !alarmON) {   //deaktiviert diesen part wenn alarm oder meldung ausgegeben wird
            if (!hMenueON && nMenueON && hMenue == 1) { //Verstellt das manuellMenü
              if (manuellMenue > 0) {
                if (manuellStart && manuellMenue == 4) manuellMenue = 2;
                else manuellMenue--;
              }
      ...
      if (!buttonPressed){
        ...
        if (!meldung && !alarmON) { //deaktiviert diesen part wenn alarm oder meldung ausgegeben wird
          ...
          //
            if (!hMenueON && nMenueON && hMenue == 1) {  //Verstellt das manuellmenü
              if (manuellMenue < 3) {
                if (manuellStart && manuellMenue == 2) manuellMenue = manuellMenue + 2;
                else manuellMenue++;
                break;
              }
            }
...
bool heizung() {
  if (autoMaischenStart || autoKochenStart || manuellStart) {
    ...
    if (millis() - heizungSTimer >= heizungSV){  //schaltet heizung ein
          if (tempT() <= (tempSoll  - hysterese)&& !heizungStatus) {
            heizungSTimer = millis();
            heizungStatus = true;
          }
      }
      if (tempT() > tempSoll && millis() - heizungSTimer >= heizungSV && heizungStatus) {  //schaltet die Heizung aus wenn temp überschwingt
            heizungSTimer = millis();
            heizungStatus = false;
      }
  }
...
void timer(){
  ...
  if (timerStart1 && tempSollErreicht && !timerEnde) { //Verstellt den Timer wenn Automatik 
    ...
    if (icurrentTimerMillis - timerPreviousMillis >= timerIntervall*2) timerPreviousMillis = icurrentTimerMillis;
    if (icurrentTimerMillis - timerPreviousMillis >= timerIntervall) {
       timerSet--;
...
void hopfentimer(){
  ...
  if (timerHopfenStart && tempSollErreicht && !timerHopfenEnde) { //Verstellt den Timer wenn Automatik 
    ...
    if (icurrentHopfenTimerMillis - timerHopfenPreviousMillis >= timerIntervall) {
       timerHopfenSet--;
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            if (hMenueON && !thingspeakData) { //hauptmenü
              client.print("</TITLE>");
              //client.print("</TITLE><meta http-equiv='refresh'content=\"30; URL=/\">");
              client.print("</HEAD><BODY>");
              // the content of the HTTP response follows the header:
              client.print("<form> <button formaction=\"/zurueck\" disabled style=\"background-color:red\">Zurueck</button></form>");
              //client.print("<br>");
              client.print("<h1>Hauptmenue</h1>");  
              client.print("<h2>Programme:</h2>");
              client.print("<form> <button formaction=\"/manuell\" style=\"width:150px\">Manueller Modus</button></form>");
              client.print("<form> <button formaction=\"/automaischen\"style=\"width:150px\">AutoMaischen Modus</button></form>");
              client.print("<form> <button formaction=\"/autokochen\"style=\"width:150px\">AutoKochen Modus</button></form>");
              client.print("<form> <button formaction=\"/thermostat\"style=\"width:150px\">Thermostat Modus</button></form>");
              client.print("<h2>Status:</h2>");
              client.print("<form> <button formaction=\"/thingspeak\"style=\"width:150px\">Thingspeak Zugangsdaten</button></form>");
              client.print("<p style =\"font-size:8pt\">&nbsp; &nbsp; &nbsp; BierBox Firmware Vers. ");
              client.print(software);
              client.print("</p>");
              client.print("<h2>Aktionen:</h2>");
              client.print("<form> <button formaction=\"/otaupdate\"style=\"width:150px;background-color:orange\">OTA Update</button></form>");
              client.print("<p style =\"font-size:8pt\">(nach dem Klick auf \"OTA Update\" <a href=\"/update\">hier</a> klicken)</p>");
              client.print("<form> <button formaction=\"/logloeschen\"style=\"width:150px;background-color:orange\">Thingspeak Log loeschen</button></form>");
              client.print("<form> <button formaction=\"/zugangloeschen\"style=\"width:150px;background-color:red\">WiFi u. Thingspeak Zugangsdaten loeschen</button></form>");
              client.print("&nbsp;<br>");
              client.print("&nbsp;<br>");
              client.print("&nbsp;<br>");
              client.print("<a style =\"font-size:8pt\" href=\"/\">AKTUALISIEREN</a><br>");
              ...
              client.print("IST-Temperatur: ");
              //char strWEB_tempT[8];
              //dtostrf(tempT(), 4, 2, strWEB_tempT);
              client.print(tempT());
              client.print(" &#xb0;C<br>");
              ...
              client.print("SOLL-Temperatur: ");
              client.print("<input autocomplete=\"off\" type=\"number\" name=\"a\" value=\"");
              client.print(manuellTemp);
              client.print("\" min=\"0\" max=\"101\">");
              client.print(" &#xb0;C<br>");
              client.print("Timer: &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;");
              client.print("<input autocomplete=\"off\" type=\"number\" name=\"b\" value=\"");
              client.print(timerManuell);
              client.print("\" min=\"1\" max=\"240\">");
              client.print(" min<br>");
              ...
              if (!manuellStart) {
                client.print("<p style =\"color:red\">Vor dem Start auf Uebernehmen klicken!</p>");
              ...
              if (!autoMaischenStart) { //automaischen einstellungen
                ...
                client.print(" &#xb0;C<br>");
                client.print("&nbsp; &nbsp;Dauer: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
                client.print("<input autocomplete=\"off\" type=\"number\" name=\"c\" value=\"");
                client.print(timerStep1);
                client.print("\" min=\"1\" max=\"120\">");
                client.print(" min&nbsp;&nbsp;&nbsp;<br>");
                client.print("&nbsp;<br>");
                client.print("Zweite Rast <br>");
                client.print("&nbsp; &nbsp;Temperatur: ");
                client.print("<input autocomplete=\"off\" type=\"number\" name=\"b\" value=\"");
                ...
                client.print("</form><br>");
                client.print("<p style =\"color:red\">Vor dem Start auf Uebernehmen klicken!</p>");
                if (!autoMaischenStart || autoMaischenEnde) client.print("<form> <button formaction=\"/start\" style=\"background-color:green\">START</button></form>");
                client.print("&nbsp;<br>");
                client.print("&nbsp;<br>");
                client.print("&nbsp;<br>");
                client.print("<a style =\"font-size:8pt\" href=\"/\">AKTUALISIEREN</a><br>");
                ...
                client.print("&nbsp;<br>");
                client.print("<h4>Maischplan:</h4>");
                client.print("Erste Rast: ");
                client.print(tempStep1);
                client.print(" &#xb0;C fuer ");
                client.print(timerStep1);
                client.print(" Minuten");
                client.print("&nbsp;<br>");
                client.print("Zweite Rast: ");
                client.print(tempStep2);
                client.print(" &#xb0;C fuer ");
                client.print(timerStep2);
                client.print(" Minuten");
              ...
              if (!autoKochenStart) { //autokochen einstellungen
                ...
                client.print("<form action=\"/a\" method=\"get\">");
                client.print("Koch-Temperatur: ");
                client.print("<input autocomplete=\"off\" type=\"number\" name=\"a\" value=\"");
                client.print(tempKochen);
                client.print("\" min=\"90\" max=\"102\">");
                client.print(" &#xb0;C");
                client.print("&nbsp;<br>");
                client.print("Koch-Dauer: &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;");
                client.print("<input autocomplete=\"off\" type=\"number\" name=\"b\" value=\"");
                client.print(timerKochen);
                client.print("\" min=\"45\" max=\"180\">");
                client.print(" min");
                client.print("&nbsp;<br>");
                client.print("<h4>Anzahl Hopfengaben:</h4>");
                client.print("<input type=\"radio\" name=\"o\" value=\"3\"");
                if (autoKochenMenue == 1 || autoKochenMenue == 3) client.print("checked");
                client.print(">1<br>");
                client.print("<input type=\"radio\" name=\"o\" value=\"5\"");
                if (autoKochenMenue == 5) client.print("checked");
                client.print(">2<br>");
                client.print("<input type=\"radio\" name=\"o\" value=\"7\"");
                if (autoKochenMenue == 7) client.print("checked");
                client.print(">3<br>");
                client.print("<input type=\"radio\" name=\"o\" value=\"9\"");
                if (autoKochenMenue == 9) client.print("checked");
                client.print(">4<br>");
                client.print("<h4>Kochplan:</h4>");
                client.print("Erste Hopfengabe:<br>");
                client.print("<input autocomplete=\"off\" type=\"number\" name=\"c\" value=\"");
                client.print(timerHopfen1);
                client.print("\" min=\"1\" max=\"120\">");
                client.print(" Minuten vor Kochende<br>");
                client.print("&nbsp;<br>");
                client.print("Zweite Hopfengabe:<br>");
                client.print("<input autocomplete=\"off\" type=\"number\" name=\"c\" value=\"");
                client.print(timerHopfen2);
                client.print("\" min=\"1\" max=\"120\">");
                client.print(" Minuten vor Kochende<br>");
                client.print("&nbsp;<br>");
                client.print("Dritte Hopfengabe:<br>");
                client.print("<input autocomplete=\"off\" type=\"number\" name=\"c\" value=\"");
                client.print(timerHopfen3);
                client.print("\" min=\"1\" max=\"120\">");
                client.print(" Minuten vor Kochende<br>");
                client.print("&nbsp;<br>");
                client.print("Vierte Hopfengabe:<br>");
                client.print("<input autocomplete=\"off\" type=\"number\" name=\"c\" value=\"");
                ...
                client.print("</form><br>");
                client.print("<p style =\"color:red\">Vor dem Start auf Uebernehmen klicken!</p>");
                if (!autoKochenStart || autoKochenEnde) client.print("<form> <button formaction=\"/start\" style=\"background-color:green\">START</button></form>");
                client.print("&nbsp;<br>");
                client.print("&nbsp;<br>");
                client.print("&nbsp;<br>");
                client.print("<a style =\"font-size:8pt\" href=\"/\">AKTUALISIEREN</a><br>");
                ...
                client.print("&nbsp;<br>");
                client.print("<h4>Kochplan:</h4>");
                client.print("Erste Hopfengabe: ");
                client.print(timerHopfen1);
                client.print(" Minuten vor Kochende");
                client.print("&nbsp;<br>"); 
                if (autoKochenMenue >= 5) {
                  client.print("Zweite Hopfengabe: ");
                  client.print(timerHopfen2);
                  client.print(" Minuten vor Kochende");
                  client.print("&nbsp;<br>"); 
                }
                if (autoKochenMenue >= 7) {
                  client.print("Dritte Hopfengabe: ");
                  client.print(timerHopfen3);
                  client.print(" Minuten vor Kochende");
                  client.print("&nbsp;<br>"); 
                }
                if (autoKochenMenue >= 9) {
                  client.print("Vierte Hopfengabe: ");
                  client.print(timerHopfen4);
                  client.print(" Minuten vor Kochende");
                  client.print("&nbsp;<br>"); 
                }
              ...
              else client.print("<form> <button formaction=\"/stop\" style=\"background-color:red\">STOP</button></form>");
              //Frame
              //https://thingspeak.com/channels/317449/charts/1?bgcolor=%23ffffff&color=%23d62020&dynamic=true&results=60&title=Temperatur&type=line
              client.print("<iframe width=\"450\" height=\"250\" style=\"border: 1px solid #cccccc;\" src=\"https://thingspeak.com/channels/");
              client.print(str_thingspeakChannelID);
              ...
              client.print("&nbsp;<br>");
              client.print("<form action=\"/a\" method=\"get\">");
              client.print("Anzeige der x-Achse ");
              client.print("<input autocomplete=\"off\" type=\"number\" name=\"a\" value=\"");
              client.print(graphTage);
            ...
            else if (hMenueON && thingspeakData) {  //thingspeak zugangsdaten
              client.print("</TITLE>");
              //client.print("</TITLE><meta http-equiv='refresh'content=\"30; URL=/\">");
              client.print("<form> <button formaction=\"/zurueck\" style=\"background-color:red\">Zurueck</button></form>");
              client.print("<h1>Thingspeak.com Einstellungen</h1>");
              client.print("&nbsp;<br>");
              client.print("<a target=\"_blank\" href=\"https://www.thingspeak.com\">www.thingspeak.com</a><br>");
              client.print("&nbsp;<br>");
              client.print("Channel ID: &nbsp;&nbsp;&nbsp;");
              client.print(str_thingspeakChannelID);
              client.print("&nbsp;<br>");
              client.print("Write API Key: ");
              client.print(str_thingspeakWriteKey);
              client.print("&nbsp;<br>");
              client.print("Read API Key: &nbsp;");
              client.print(str_thingspeakReadKey);
              client.print("&nbsp;<br>");
              client.print("User Key: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;");
        ...
        if (hMenueON && !thingspeakData) { //hauptmenü
          ...
          if (currentLine.endsWith("GET /thingspeak")) {
            thingspeakData = true;
          }
        ...
        if (hMenueON && thingspeakData) { //tingspeak zugangsdaten
          if (currentLine.endsWith("GET /zurueck") && !gotValue) {
          thingspeakData = false;
          }
        }
      }
      delay(1);
...
void thingspeakLog () {
    ...
    if (millis() - thingspeakPreviousMillis >= thingspeakIntervall && client.connect(serverThingspeak,80)) {
      ...
      if (thingspeakChannelClear) {
         ...
         thingspeakTime += thingspeakIntervall/1000;
         String str_thingspeakTime = itoa(thingspeakTime, buffer, 10);
         postStr = "api_key="+str_thingspeakWriteKey+"&field2="+str_tempT+"&field1="+str_thingspeakTime;
      }
    ...
    //WIFI
      ...
      if (SPIFFS.begin()) {
        ...
        if (SPIFFS.exists("/configBierBox.json")) {
          ...
          File configFile = SPIFFS.open("/configBierBox", "r");
          if (configFile) {
      ...
      if (shouldSaveConfig) {
        ...
        File configFile = SPIFFS.open("/configBierBox", "w");
        if (!configFile) {
...
void loop() {
  ...
  //
  //Schaltet webThermostat aus und ein
